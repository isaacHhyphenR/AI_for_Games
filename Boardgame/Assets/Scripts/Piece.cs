...
public struct PiecePosition
{
    ...
    public PiecePosition(Coordinate _headPosition, Direction _direction)
    {
    ...
    {
        return lhs.headPosition == rhs.headPosition && lhs.direction == rhs.direction;
    }
    public static bool operator !=(PiecePosition lhs, PiecePosition rhs)
    {
        return !(lhs == rhs);
...
public class Piece : MonoBehaviour
{
            ...
            if(GameManager.GetSelectedPiece() == this && mouseDown && GameManager.canPlay)
            {
                ...
                if (Physics.Raycast(ray, out hit, 100, GameManager.boardClickMask))
                {
                    ...
                    if (CanRotateToSpace(destination) || CanDashToSpace(destination))
                    {
                        ...
                        {
                            moveDirection = GridManager.DirectionBetweenSquares(destination, board.GetSquare(GetHeadLocation()));
                            destination = board.GetSquare(GetHeadLocation());
                        }
                        SetPosition(destination.GetCoordinates(),moveDirection);
                        EndMove();
                    }
                }
            }
    ...
    {
        return (CanRotateFromTo(board.GetSquare(GetHeadLocation()), destination, true) || CanRotateFromTo(GetTailLocation(), destination, false));
    }
    /// <summary>
    ...
    bool CanDashToSpace(GridSquare destination)
    {
        ...
        {
            return !MoveBlockedByPiece(board.GetSquare(GetHeadLocation()), destination, destination, direction);
        }
        return false;
    }
    ...
    {
        Collision collision = piece.board.FirstPieceEncountered(start, direction, GridManager.DistanceToSquare(start, destination));
        ...
        if (collision.piece.GetOwner() != piece.owner &&
            collision.piece.GetHeadLocation() != collision.square.GetCoordinates() && collision.square == headDestination)
        {
            return false;
        }
        return true;
    ...
    {
        SetPosition(position.headPosition, position.direction);
    }
    /// <summary>
    /// Places the piece's head on the designated square with the head facing in the designated direction
    /// </summary>
    ...
    public void SetPosition(Coordinate headPosition, Direction headDirection)
    {
        ...
        if(canWin && GetHeadLocation().y == GameManager.GetOtherPlayer(owner).GetHomeRow())
        {
    ...
    /// <returns></returns>
    public List<Move> GetAllMoves(Coordinate startPosition, Direction startDirection, bool calculateWeights, bool includeBlockedByPieces = false)
    {
        GridSquare startHead = board.grid[startPosition.x, startPosition.y];
        GridSquare startTail = board.SquareInDirection(startHead, GridManager.OppositeDirection(startDirection), GetLength() - 1);
        ...
        for(int i = 0; i < 4; i++)
        {
            ...
            {
                GridSquare destination = board.SquareInDirection(startHead, GridManager.OppositeDirection(newDir), length - 1);
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (GridManager.AreDirectionsAdjacent(startDirection, newDir))
            {
                GridSquare destination = board.SquareInDirection(startTail, newDir, length - 1);
        ...
        GridSquare[] dashSquares = board.SquaresInDirection(startHead, startDirection, 100, false);
    ...
    public bool AnyValidMove()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                GridSquare destination = board.SquareInDirection(GetHeadSquare(), GridManager.OppositeDirection(newDir), length - 1);
                if (destination != null && !MoveBlockedByPiece(GetHeadSquare(), destination, GetHeadSquare(), GridManager.OppositeDirection(newDir)))
                {
                    return true;
                }
            }
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (GridManager.AreDirectionsAdjacent(direction, newDir))
            {
                GridSquare destination = board.SquareInDirection(GetTailLocation(), newDir, length - 1);
        ...
        //Checks all possible dashes
        GridSquare[] dashSquares = board.SquaresInDirection(GetHeadSquare(), direction, 100, false);
        for (int i = 0; i < dashSquares.Length; i++)
        {
            if (!MoveBlockedByPiece(GetHeadSquare(), dashSquares[i], dashSquares[i], direction))
            {
                return true;
            }
        }
    ...
    {
        return currentSquares.First().GetCoordinates();
    }
    ...
    {
        return new PiecePosition(GetHeadLocation(), GetDirection());
    }
    /// <summary>
    /// Sets which BoardState this piece exists on
    /// </summary>
    /// <param name="_board"></param>
    public void SetBoard(BoardState _board)
    {
        board = _board;
    }

    public GridSquare GetHeadSquare(BoardState _board)
    {
        return _board.GetSquare(GetHeadLocation());
    }
    public GridSquare GetHeadSquare()
    {
        return GetHeadSquare(board);
    }
}
