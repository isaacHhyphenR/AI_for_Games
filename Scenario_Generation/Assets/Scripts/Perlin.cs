...
public class Perlin : MonoBehaviour
{
    ...
    const int SIZE = 256; //length of permutaiton
    const int MASK = SIZE - 1;
    static float[] grads_x = new float[SIZE];
    static float[] grads_y = new float[SIZE];

    /// <summary>
    /// Initialises the unit vectors
    /// </summary>
    public static void Init()
    {
        for (int i = 0; i < SIZE; ++i)
        {
            grads_x[i] = Mathf.Cos(2 * Mathf.PI * i / SIZE);
            grads_y[i] = Mathf.Sin(2 * Mathf.PI * i / SIZE);
        }
    }
    ...
    static float surflet(float x, float y, float grad_x, float grad_y)
    {
